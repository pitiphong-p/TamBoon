...
public func ==(lhs: PaymentInformation, rhs: PaymentInformation) -> Bool {
  return lhs.token == rhs.token && lhs.amount == rhs.amount
}

extension PaymentInformation: Hashable {
  public var hashValue: Int {
    return ~token.hashValue ^ amount.hashValue
  }
}
...
public func ==(lhs: TamBoonAPIError, rhs: TamBoonAPIError) -> Bool {
  switch (lhs, rhs) {
  case (.HTTPError(let lhsStatusCode, message: let lhsMessage), .HTTPError(let rhsStatusCode, message: let rhsMessage)):
...
extension TamBoonAPIError: Hashable {
  public var hashValue: Int {
    switch self {
    case .HTTPError(let statusCode, message: let message):
      return ~statusCode.hashValue ^ (message?.hashValue ?? 0)
    case .FoundationError(let error):
      return ~error.hashValue
    case .InvalidResponse:
      return ~"InvalidResponse".hashValue
    }
  }
}
